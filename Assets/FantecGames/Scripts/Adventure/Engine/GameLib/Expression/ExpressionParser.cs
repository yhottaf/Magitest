...
namespace fantec
{
    ...
    public class ExpressionParser
    {
        ...
        public object CalcExp(System.Func<string, object> callbackGetValue, System.Func<string, object, bool> callbackSetValue)
        {
            ...
            foreach (ExpressionToken token in tokens)
            {
                if (token.Type == ExpressionToken.TokenType.Value)
                {
                    ...
                    if (obj == null)
                    {
                        AddErrorMsg(LanguageErrorMsg.LocalizeTextFormat(fantec.ErrorMsg.ExpUnknownParameter, token.Name));
                        isError = true;
                    }
                    else
                    {
        ...
        public bool CalcExpBoolean(System.Func<string, object> callbackGetValue, System.Func<string, object, bool> callbackSetValue)
        {
            ...
            AddErrorMsg(LanguageErrorMsg.LocalizeTextFormat(fantec.ErrorMsg.ExpResultNotBool));
            return false;
        }
        ...
        object Calc(System.Func<string, object, bool> callbackSetValue)
        {
            try
            {
                ...
                if (values.Count != 1)
                {
                    throw new Exception(LanguageErrorMsg.LocalizeTextFormat(fantec.ErrorMsg.ExpIllegal));
                }
        ...
        List<ExpressionToken> ToReversePolishNotation(string exp)
        {
            ...
            {
                AddErrorMsg(LanguageErrorMsg.LocalizeTextFormat(fantec.ErrorMsg.ExpIllegal));
            }
                ...
                try
                {
                    switch (token.Type)
                    {
                        ...
                        case ExpressionToken.TokenType.Comma:   //カンマ
                            ...
                            break;
                        default:
                            AddErrorMsg(LanguageErrorMsg.LocalizeTextFormat(fantec.ErrorMsg.UnknownType, token.Type.ToString()));
                            break;
                    }
                }
