...
namespace fantec
{
    public class CharData : IParsedTextData
    {
        ...
        public class CustomCharaInfo
        {
            ...
            // 直前のデータと比べて、Boldのカスタム設定が終了するかどうか
            ...
            // 直前のデータと比べて、Boldのカスタム設定が開始するかどうか
            public bool IsBeginBold(CustomCharaInfo lastCustomInfo)
            {
                if (!IsBold) return false;
                return (!lastCustomInfo.IsBold);
            }
            // 直前のデータと比べて、Italicのカスタム設定が終了するかどうか
            public bool IsEndItalic(CustomCharaInfo lastCutomInfo)
            {
                if (!lastCutomInfo.IsItalic) return false;
                return (!IsItalic);
            }

            // 直前のデータと比べて、Italicのカスタム設定が開始するかどうか
            public bool IsBeginItalic(CustomCharaInfo lastCutomInfo)
            {
                if (!IsItalic) return false;
                return (!lastCutomInfo.IsItalic);
            }

            // 直前のデータと比べて、サイズのカスタム設定が終了するかどうか
            public bool IsEndSize(CustomCharaInfo lastCutomInfo)
            {
                if (!lastCutomInfo.IsSize) return false;
                if (!IsSize) return true;
                return lastCutomInfo.size != size;
            }

            // 直前のデータと比べて、サイズのカスタム設定が開始するかどうか
            public bool IsBeginSize(CustomCharaInfo lastCutomInfo)
            {
                if (!IsSize) return false;
                if (!lastCutomInfo.IsSize) return true;
                return lastCutomInfo.size != size;
            }

            // 直前のデータと比べて、カラーのカスタム設定が終了するか
            public bool IsEndColor(CustomCharaInfo lastCutomInfo)
            {
                if (!lastCutomInfo.IsColor) return false;
                if (!IsColor) return true;
                return lastCutomInfo.color != color;
            }
            // 直前のデータと比べて、カラーのカスタム設定が開始するかどうか
            public bool IsBeginColor(CustomCharaInfo lastCutomInfo)
            {
                if (!IsColor) return false;
                if (!lastCutomInfo.IsColor) return true;
                return lastCutomInfo.color != color;
            }
