...
namespace fantec
{
    ...
    public static class AdvCommandParser
    {
        ...
        static public AdvCommand CreateCommand(StringGridRow row, AdvSettingDataManager dataManager)
        {
            ...
            {
                //列名がついたセル全て空かどうか
                if (row.IsAllEmptyCellNamedColum())
                {
                }
                else
                {
        ...
        static AdvCommand CreateCommandDefault(string id, StringGridRow row, AdvSettingDataManager dataManager)
        {
            switch (id)
            {
                ...
                case IdShowMessageWindow:
                    return new AdvCommandShowMessageWindow
                        (row);
        ...
        static string ParseCommandID(StringGridRow row)
        {
            ...
            string id = AdvParser.ParseCellOptional<string>(row, AdvColumName.Command, "");
            if (string.IsNullOrEmpty(id))
            {
                //コマンドなしは、テキスト表示が基本
                if (!AdvParser.IsEmptyCell(row, AdvColumName.Arg1))
                {
                    Profiler.EndSample();
                    //パラメーターつきなので、キャラ表示
                    return IdCharacter;
        ...
        //
        static public bool TryParseScenarioLabel(StringGridRow row, AdvColumName columnName, out string scenarioLabel)
        {
        ...
        //シナリオラベルを解析・取得
        static public string ParseScenarioLabel(StringGridRow row, AdvColumName name)
        {
            string label;
        ...
        {
            string command = AdvParser.ParseCellOptional<string>(row, AdvColumName.Command, "");
