    ...
    public class AdvMacroManager
    {
        ...
        public bool TryAddMacroData(string name, StringGrid grid)
        {
            ...
            while (index < grid.Rows.Count)
            {
                ...
                if (row.IsEmptyOrCommentOut) continue;                              //データがない
                ...
                if (TryParseMacoBegin(row, out macroName))
                {
                    ...
                    while (index < grid.Rows.Count)
                    {
                        ...
                        ++index;
                        if (macroRow.IsEmptyOrCommentOut) continue;                     //データがない
                        if (AdvParser.ParseCellOptional<string>(macroRow, AdvColumName.Command, "") == "EndMacro")
                        {
                            break;
                        }
        ...
        {
            string commandName = AdvParser.ParseCellOptional<string>(row, AdvColumName.Command, "");
            AdvMacroData data;
        ...
        {
            return AdvCommandParser.TryParseScenarioLabel(row, AdvColumName.Command, out macroName);
        }
    }
}