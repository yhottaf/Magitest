    ...
    public class AdvEntityData
    {
        ...
        public static bool TryCreateEntityStrings(StringGridRow original, System.Func<string, object> GetParameter, out string[] strings)
        {
            ...
            for (int i = 0; i < original.Strings.Length; ++i)
            {
                ...
                int indexWindowType;
                if (original.Grid.TryGetColumIndex(AdvColumName.WindowType.QuickToString(), out indexWindowType))
                {
                    if (i == indexWindowType)
                    {
                        Debug.LogError(" Can not use entity in " + AdvColumName.WindowType.QuickToString());
                        return false;
                    }
                }
                int indexPageCtrl;
                if (original.Grid.TryGetColumIndex(AdvColumName.PageCtrl.QuickToString(), out indexPageCtrl))
                {
                    if (i == indexPageCtrl)
                    {
                        Debug.LogError(" Can not use entity in " + AdvColumName.PageCtrl.QuickToString());
                        return false;
                    }
                }
