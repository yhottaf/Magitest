...
namespace fantec
{
    ...
    public class AdvLayerSettingData : AdvSettingDictinoayItemBase
    {
        ...
        {
            RowData = row;
            string key = AdvParser.ParseCell<string>(row, AdvColumName.LayerName);
            ...
            {
                InitKey(key);
                this.Type = AdvParser.ParseCell<LayerType>(row, AdvColumName.Type);
                if (this.Type == LayerType.Dummy)
                {
                    return true;
                }
                this.Order = AdvParser.ParseCell<int>(row, AdvColumName.Order);
                this.LayerMask = AdvParser.ParseCellOptional<string>(row, AdvColumName.LayerMask, "");

                //X座標や幅の設定
                this.Horizontal = new RectSetting();
                bool isBorderLeft = !AdvParser.IsEmptyCell(row, AdvColumName.BorderLeft);
                bool isBorderRight = !AdvParser.IsEmptyCell(row, AdvColumName.BorderRight);
                if (isBorderLeft)
                {
                ...
                Horizontal.position = AdvParser.ParseCellOptional<float>(row, AdvColumName.X, 0);
                Horizontal.size = AdvParser.ParseCellOptional<float>(row, AdvColumName.Width, 0);
                Horizontal.borderMin = AdvParser.ParseCellOptional<float>(row, AdvColumName.BorderLeft, 0);
                Horizontal.borderMax = AdvParser.ParseCellOptional<float>(row, AdvColumName.BorderRight, 0);


                //Y座標や高さの設定
                this.Vertical = new RectSetting();
                bool isBorderTop = !AdvParser.IsEmptyCell(row, AdvColumName.BorderTop);
                bool isBorderBottom = !AdvParser.IsEmptyCell(row, AdvColumName.BorderBottom);
                if (isBorderTop)
                {
                ...
                Vertical.position = AdvParser.ParseCellOptional<float>(row, AdvColumName.Y, 0);
                Vertical.size = AdvParser.ParseCellOptional<float>(row, AdvColumName.Height, 0);
                Vertical.borderMin = AdvParser.ParseCellOptional<float>(row, AdvColumName.BorderBottom, 0);
                Vertical.borderMax = AdvParser.ParseCellOptional<float>(row, AdvColumName.BorderTop, 0);


                Vector2 pivot;
                pivot.x = AdvParser.ParseCellOptional<float>(row, AdvColumName.PivotX, 0.5f);
                pivot.y = AdvParser.ParseCellOptional<float>(row, AdvColumName.PivotY, 0.5f);
                this.Pivot = pivot;

                Vector3 scale;
                scale.x = AdvParser.ParseCellOptional<float>(row, AdvColumName.ScaleX, 1.0f);
                scale.y = AdvParser.ParseCellOptional<float>(row, AdvColumName.ScaleY, 1.0f);
                scale.z = AdvParser.ParseCellOptional<float>(row, AdvColumName.ScaleZ, 1.0f);
                this.Scale = scale;

                this.Z = AdvParser.ParseCellOptional<float>(row, AdvColumName.Z, -0.01f * Order);
                this.Alignment = AdvParser.ParseCellOptional<Alignment>(row, AdvColumName.Align, Alignment.None);
                this.FlipX = AdvParser.ParseCellOptional<bool>(row, AdvColumName.FlipX, false);
                this.FlipY = AdvParser.ParseCellOptional<bool>(row, AdvColumName.FlipY, false);
                return true;
            }
        }
