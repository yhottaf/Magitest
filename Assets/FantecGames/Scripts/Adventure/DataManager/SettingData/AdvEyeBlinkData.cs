...
namespace fantec
{
    ...
    public class AdvEyeBlinkData : AdvSettingDictinoayItemBase
    {
        ...
        public override bool InitFromStringGridRow(StringGridRow row)
        {
            ...
            InitKey(key);
            this.IntervalMin = AdvParser.ParseCellOptional<float>(row, AdvColumName.IntervalMin, 2);
            this.IntervalMax = AdvParser.ParseCellOptional<float>(row, AdvColumName.IntervalMax, 6);
            this.RandomDoubleEyeBlink = AdvParser.ParseCellOptional<float>(row, AdvColumName.RandomDouble, 0.2f);
            this.Tag = AdvParser.ParseCellOptional<string>(row, AdvColumName.Tag, "eye");
