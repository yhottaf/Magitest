...
namespace fantec
{
    ...
    public class AdvGraphicInfo : IAssetFileSettingData
    {
        ...
        public AdvGraphicInfo(string dataType, int index, string key, StringGridRow row, IAdvSettingData advSettindData)
        {
            ...
            switch (DataType)
            {
                case AdvGraphicInfo.TypeParticle:
                    ...
                    break;
                default:
                    this.FileType = AdvParser.ParseCellOptional<string>(row, AdvColumName.FileType, "");
                    break;
            }

            this.FileName = AdvParser.ParseCell<string>(row, AdvColumName.FileName);
            try
            {
                this.Pivot = ParserUtil.ParsePivotOptional(AdvParser.ParseCellOptional<string>(row, AdvColumName.Pivot, ""), new Vector2(0.5f, 0.5f));
            }
            ...
            try
            {
                string pivot0String = AdvParser.ParseCellOptional<string>(row, AdvColumName.Pivot0, "");
            ...
            try
            {
                this.Scale = ParserUtil.ParseScale3DOptional(AdvParser.ParseCellOptional<string>(row, AdvColumName.Scale, ""), Vector3.one);
            }
            catch (System.Exception e)
            {
                Debug.LogError(row.ToErrorString(e.Message));
            ...
            Vector3 pos;
            pos.x = AdvParser.ParseCellOptional<float>(row, AdvColumName.X, 0);
            pos.y = AdvParser.ParseCellOptional<float>(row, AdvColumName.Y, 0);
            pos.z = AdvParser.ParseCellOptional<float>(row, AdvColumName.Z, 0);
            this.Position = pos;

            this.SubFileName = AdvParser.ParseCellOptional<string>(row, AdvColumName.SubFileName, "");
            this.AnimationState = AdvParser.ParseCellOptional<string>(row, AdvColumName.AnimationState, "");

            this.ConditionalExpression = AdvParser.ParseCellOptional<string>(row, AdvColumName.Conditional, "");
        ...
        public void BootInit(Func<string, string, string> FileNameToPath, AdvSettingDataManager dataManager)
        {
            ...
            //アニメーションの設定
            string animationLabel = AdvParser.ParseCellOptional<string>(RowData, AdvColumName.Animation, "");
            ...
            //目パチの設定
            string eyeBlinkLabel = AdvParser.ParseCellOptional<string>(RowData, AdvColumName.EyeBlink, "");
            ...
            //口パクの設定
            string lipSynchLabel = AdvParser.ParseCellOptional<string>(RowData, AdvColumName.LipSynch, "");
