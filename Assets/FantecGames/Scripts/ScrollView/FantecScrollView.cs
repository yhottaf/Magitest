...
namespace FantecScrollView
{
    ...
    public abstract class FantecScrollView<TItemData, TContext> : MonoBehaviour where TContext : class, new()
    {
        ...
        void ResizePool(float firstPosition)
        {
            ...
            {
                var cell = Instantiate(CellPrefab, cellContainer).GetComponent<FantecCell<TItemData, TContext>>();
                if (cell == null)
                {
                    throw new MissingComponentException(string.Format(
                        "FantecCell<{0}, {1}> component not found in {2}.",
    ...
    public abstract class FantecScrollView<TItemData> : FantecScrollView<TItemData, NullContext> { }
}
