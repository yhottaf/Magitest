...
{
    public class ScreenSwipeObserver : MonoBehaviour
    {
        [SerializeField, Range(1e-2f, 100f)]
        private float ThresholdSeconds = 1.0f;
        [SerializeField, Range(1e-2f, 1000f)]
        ...
        private void Start()
        {
            ...
            var onEndDragObservable = eventTrigger
                ...
                .TakeUntilDisable(this)
                .Where(eventData => (DateTime.Now - this.beginTime).TotalSeconds < ThresholdSeconds)
                .Select(eventData => eventData.position)
                .Share();
